...
const { getAuthClient, getDriveAuthClient } = require('../config/googleClients');
...
// Client Drive RIÊNG cho các thao tác TẠO FILE/FOLDER MỚI (OAuth — xem giải thích trong
// config/googleClients.js:getDriveAuthClient). Đọc file (taiAnhTuLinkDrive) vẫn dùng service
// account như cũ vì đó không phải tạo file mới, không đụng tới quota.
async function getDriveClientTaoMoi() {
...
async function getOrCreateFolder(name, parentId = ROOT_FOLDER_ID) {
  const drive = await getDriveClientTaoMoi();
